...
% ag_name = 'ag_se_fdtd_20_ZeroPot.gif';
 ag_name = 'ag_se_fdtd_20_EField.gif';
...
% (4) hill U0 > 0/ well U) < 0   (5)  parabolic well
...
% ADDED FOR DOUBLE BARRIER ONLY
% When flagU = 6, use positive potential-axis limits so the barriers are visible.
if flagU == 6
   U1 = -10;
   U2 = 70;
   UyTick = [0 62];
...
% Potential energy function U  -------------------------------------------
   ...
   switch flagU
     ...
     case 6                   % double rectangular barrier
         % ADDED FOR DOUBLE BARRIER ONLY
         % Two barriers of height +100 eV with a central well between them.
         U(:) = 0;

         DB_height = 62;       % barrier height [eV] - slightly above incident KE
         DB_width  = 18;       % barrier width [grid points] ~= 0.072 nm
         DB_well   = 70;       % central well width [grid points] ~= 0.28 nm
